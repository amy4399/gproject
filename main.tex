...
% ================ 脚注格式设置 ================
% 脚注字号小五，中文宋体英文Times New Roman
\renewcommand{\footnotesize}{\五号}
\let\oldfootnote\footnote
\renewcommand{\footnote}[1]{\oldfootnote{\songti#1}}

% ================ 图表格式设置 ================
% 图题、表题格式：中文黑体五号加粗，英文Times New Roman五号加粗，居中
% 表序表题之间空一格，不加标点
...
\captionsetup{labelsep=enspace} % 表序表题之间空一格
\captionsetup[subfigure]{labelformat=rightBracket, labelsep=enspace} % 子图a,b,c外没有括号
\captionsetup[table]{capposition=top} % 表题在上方
\captionsetup[figure]{capposition=bottom} % 图题在下方
\ctexset{
    figurename = {图},
    tablename  = {表},
    listfigurename = {插图},
...
\section{引言}
在此处输入引言内容。介绍研究背景、目的和意义。\footnote{这是一个脚注示例，用来补充说明正文内容。脚注字号小五，中文宋体英文Times New Roman。}
...
\section{实验结果与分析}
在此处展示实验结果并进行分析。表格与图片可以通过 \verb|\ref{<key>}| 来引用，例如表~\ref{table1}、表~\ref{table2}、图~\ref{fig1}。
...
\begin{table}[htb]
  \centering
  \caption{表格标题在上方，表序表题之间空一格，不加标点}
  \label{table1}
  \begin{tabular}{lc}
    \toprule
    Example & Result \\
...
\begin{figure}[htb]
  ...
  \caption{图片标题在下方，图序图题之间空一格，不加标点}\label{fig1}
\end{figure}
...
\begin{figure}[htb]
  ...
  \begin{subfigure}[t]{.45\linewidth}
    ...
    \caption{子图标题1}\label{fig2a}
  \end{subfigure}
  \begin{subfigure}[t]{.45\linewidth}
    \centering
    \includegraphics[width=1\textwidth]{example-image-b}
    \caption{子图标题2}\label{fig2b}
  \end{subfigure}
  \caption{包含子图的图片，子图用a、b、c标序}\label{fig2}
\end{figure}
